...
#include <proxy/ProxyJSDialogCallback.h>
...
void EDITIONF(ProxyJSDialogCallback_Destructor)(PMDATA_INF pRetData, INT nArgCount, PMDATA_INF pArgInf){
	...
	shrewd_ptr<ProxyJSDialogCallback> ptr = (ProxyJSDialogCallback*)*pArgInf->m_ppCompoundData;
...
void EDITIONF(ProxyJSDialogCallback_CopyConstructor)(PMDATA_INF pRetData, INT nArgCount, PMDATA_INF pArgInf){
	...
	shrewd_ptr<ProxyJSDialogCallback> ptr = (ProxyJSDialogCallback*)*pArgInf[1].m_ppCompoundData;
...
void EDITIONF(ProxyJSDialogCallback_Continue) (PMDATA_INF pRetData, INT nArgCount, PMDATA_INF pArgInf){
	...
	shrewd_ptr<ProxyJSDialogCallback> self = (ProxyJSDialogCallback*)*pArgInf->m_ppCompoundData;
	bool argSuccess = pArgInf[1].m_bool;
	const char* argUser_Input = (NULL==pArgInf[2].m_pText || strlen(pArgInf[2].m_pText) <= 0) ? NULL : pArgInf[2].m_pText;
	self->Continue(argSuccess,argUser_Input);
}
