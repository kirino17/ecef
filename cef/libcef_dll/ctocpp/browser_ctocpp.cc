...
//
// $hash=683f760ed8371b6edc7d4950203f7ae2e32d0321$
//
...
NO_SANITIZE("cfi-icall")
void CefBrowserCToCpp::SetTouchEventEmulationEnabled(bool enabled,
                                                     int maxTouchPoints) {
  shutdown_checker::AssertNotShutdown();

  cef_browser_t* _struct = GetStruct();
  if (CEF_MEMBER_MISSING(_struct, set_touch_event_emulation_enabled))
    return;

  // AUTO-GENERATED CONTENT - DELETE THIS COMMENT BEFORE MODIFYING

  // Unverified params: maxTouchPoints

  // Execute
  _struct->set_touch_event_emulation_enabled(_struct, enabled, maxTouchPoints);
}

NO_SANITIZE("cfi-icall")
void CefBrowserCToCpp::SetGeolocationOverride(float latitude,
                                              float longitude,
                                              float accuracy) {
  shutdown_checker::AssertNotShutdown();

  cef_browser_t* _struct = GetStruct();
  if (CEF_MEMBER_MISSING(_struct, set_geolocation_override))
    return;

  // AUTO-GENERATED CONTENT - DELETE THIS COMMENT BEFORE MODIFYING

  // Unverified params: latitude, longitude, accuracy

  // Execute
  _struct->set_geolocation_override(_struct, latitude, longitude, accuracy);
}

NO_SANITIZE("cfi-icall")
void CefBrowserCToCpp::SetEmulatedMedia(
    const CefString& media,
    CefRefPtr<CefDictionaryValue> features) {
  shutdown_checker::AssertNotShutdown();

  cef_browser_t* _struct = GetStruct();
  if (CEF_MEMBER_MISSING(_struct, set_emulated_media))
    return;

  // AUTO-GENERATED CONTENT - DELETE THIS COMMENT BEFORE MODIFYING

  // Verify param: media; type: string_byref_const
  DCHECK(!media.empty());
  if (media.empty())
    return;
  // Verify param: features; type: refptr_same
  DCHECK(features.get());
  if (!features.get())
    return;

  // Execute
  _struct->set_emulated_media(_struct, media.GetStruct(),
                              CefDictionaryValueCToCpp::Unwrap(features));
}

NO_SANITIZE("cfi-icall") void CefBrowserCToCpp::ClearMediaFeatureOverrides() {
  shutdown_checker::AssertNotShutdown();

  cef_browser_t* _struct = GetStruct();
  if (CEF_MEMBER_MISSING(_struct, clear_media_feature_overrides))
    return;

  // AUTO-GENERATED CONTENT - DELETE THIS COMMENT BEFORE MODIFYING

  // Execute
  _struct->clear_media_feature_overrides(_struct);
}

NO_SANITIZE("cfi-icall")
void CefBrowserCToCpp::EnableDeviceEmulation(
    CefRefPtr<CefDeviceEmulationParams> params) {
  shutdown_checker::AssertNotShutdown();

  cef_browser_t* _struct = GetStruct();
  if (CEF_MEMBER_MISSING(_struct, enable_device_emulation))
    return;
  ...
  // Execute
  _struct->enable_device_emulation(
      _struct, CefDeviceEmulationParamsCToCpp::Unwrap(params));
}

NO_SANITIZE("cfi-icall") void CefBrowserCToCpp::DisableDeviceEmulation() {
  shutdown_checker::AssertNotShutdown();

  cef_browser_t* _struct = GetStruct();
  if (CEF_MEMBER_MISSING(_struct, disable_device_emulation))
    return;

  // AUTO-GENERATED CONTENT - DELETE THIS COMMENT BEFORE MODIFYING

  // Execute
  _struct->disable_device_emulation(_struct);
}
